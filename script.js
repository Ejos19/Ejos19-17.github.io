...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderHistory() {
    ...
    // Si el arreglo no contiene registros, se muestra un mensaje informativo en una fila completa
    if (history.length === 0) {
    ...
    history.forEach((item) => {
      ...
      tr.innerHTML = `
        ...
        <td class="history-usd">${item.usd}</td>
        <td class="history-eur">${item.eur}</td>
      `;
